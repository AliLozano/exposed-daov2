...
class OneToManyQuery<ID : Comparable<ID>, E : Entity<ID>, T : EntityManager<ID, E, T>>(entityManager: T,
                                                                                       rawQuery: Query) : EntityQueryBase<ID, E, T>(entityManager, rawQuery),
        Iterable<E>

class OneToManyRelationRef<ID : Comparable<ID>, E : Entity<ID>, M : EntityManager<ID, E, M>>(val ref: M, val column: Column<Any>) {
    operator fun <ID : Comparable<ID>, E : Entity<ID>, M2 : EntityManager<ID, E, M2>> getValue(table: EntityManager<ID, E, M2>, property: KProperty<*>): M {
